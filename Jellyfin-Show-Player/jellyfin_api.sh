...
jellyfin_get_shows() {
    ...
    curl -k -s "$SERVER_URL/Items?IncludeItemTypes=Series&Recursive=true&Fields=PrimaryImageAspectRatio,Path&UserId=$user_id" -H "$(get_auth_header)"
}

jellyfin_get_seasons() {
    local series_id="$1"
    load_config
    local user_id=$(get_user_id)
    
    [ -z "$series_id" ] && return 1
    
    curl -k -s "$SERVER_URL/Shows/$series_id/Seasons?UserId=$user_id&Fields=PrimaryImageAspectRatio,Path" -H "$(get_auth_header)"
}
...
jellyfin_get_episodes() {
    ...
    [ -z "$series_id" ] || [ -z "$season_id" ] && return 1
    
    curl -k -s "$SERVER_URL/Shows/$series_id/Episodes?SeasonId=$season_id&UserId=$user_id&Fields=PrimaryImageAspectRatio,Path" -H "$(get_auth_header)"
}
...
parse_show_list() {
    ...
    if [ -f "$input_file" ]; then
        jq -r '.Items[] | "\(.Id)|\(if .IndexNumber then ("S" + (if .ParentIndexNumber then (.ParentIndexNumber | tostring) else "00" end | if length == 1 then ("0" + .) else . end) + "E" + (.IndexNumber | tostring | if length == 1 then ("0" + .) else . end) + " - ") else "" end)\(.Name)"' "$input_file"
    fi
}
